  ...
  return (
    <div className="mx-auto px-3 mt-8 mb-4">
        ...
        <div className={`grid grid-cols-1 ${sections.length + (moreSection ? 1 : 0) >= 3 ? 'md:grid-cols-3' : sections.length + (moreSection ? 1 : 0) >= 2 ? 'md:grid-cols-2' : ''} gap-4 mb-6`}>
          {sections.map(section => (
            <div key={section.title} className="rounded-2xl overflow-hidden" style={{ boxShadow: 'var(--box-shadow-strong)' }}>
              <h3 className="bg-primary text-primary-foreground text-lg font-bold py-2.5 px-4">{section.title}</h3>
              <div className="bg-background py-2 px-4">
                {section.items.map((item, i) => (
                  <div key={i} className="py-1.5">
                    {item.url ? (
                      <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-primary text-base font-medium hover:underline hover:text-accent hover:pl-1 transition-all">
                        {item.text}
                      </a>
                    ) : (
          ...
          {moreSection && (
            <div className="rounded-2xl overflow-hidden" style={{ boxShadow: 'var(--box-shadow-strong)' }}>
              <h3 className="bg-primary text-primary-foreground text-lg font-bold py-2.5 px-4">{moreSection.title}</h3>
              <div className="bg-background py-2 px-4">
                <div className="grid grid-cols-3 gap-x-4 gap-y-1.5">
                  {moreSection.items.map((item, i) => (
                    <a
                      ...
                      className="text-primary text-base font-medium hover:underline hover:text-accent hover:pl-1 transition-all cursor-pointer py-1"
                    >
                      {item.text}
                    </a>
                  ))}
      ...
      <div className="rounded-2xl overflow-hidden mb-6" style={{ boxShadow: 'var(--box-shadow-strong)' }}>
        <h2 className="bg-primary text-primary-foreground text-lg font-bold py-3 px-5">
          Welcome to the Official Website of Sarkari Sewayojan
        </h2>
        <div className="bg-background text-primary py-3 px-5 text-base leading-relaxed">
        ...
        <div className="bg-background text-primary py-3 px-5 text-base leading-relaxed border-t border-border">
